...
def Inference(FLAGS):
    ...
    test_save_path = "../model/{}_{}_labeled/{}_predictions333/".format(FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
    ...
    save_mode_path = os.path.join(snapshot_path, '{}_best_model_us24.pth'.format(FLAGS.model))
    ...
    # 用于保存每个病例的 ASD 值，列表中每个元素为 [case, ASD_Average]
    asd_results = []

    for case in tqdm(image_list):
        first_metric, second_metric, third_metric = test_single_volume(case, net, test_save_path, FLAGS)
        # 计算三个类别的 ASD 均值（各类别指标中的第二项）
        avg_asd = (first_metric[0] + second_metric[0] + third_metric[0]) / 3.0
        asd_results.append([case, avg_asd])
    ...
    # 将 ASD 结果保存为 CSV 文件
    csv_path = os.path.join(test_save_path, "Dice_results.csv")
    with open(csv_path, mode='w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(["Case", "Dice_Average"])
        csv_writer.writerows(asd_results)
    print("ASD results have been saved to:", csv_path)

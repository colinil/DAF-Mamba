...
# 新增：输出图像保存目录参数
parser.add_argument('--output_dir', type=str, default='/home/colin/Mamba-UNet-main/model/cam/test', help='Directory to save output images')
...
model.load_state_dict(torch.load("/home/colin/Mamba-UNet-main/model/MnMs/VIM_150_labeled/mambaunet/mambaunet_best_model_us24.pth"))
model.eval()
...
class GradCAM:
    ...
    def forward_hook(self, module, input, output):
        # 如果 output 是 (B,H,W,C)，转 (B,C,H,W)
        if output.dim() == 4 and output.shape[1] != output.shape[-1]:
    ...
    def backward_hook(self, module, grad_input, grad_output):
        grad = grad_output[0]
        # 同理 (B,H,W,C) -> (B,C,H,W)
        if grad.dim() == 4 and grad.shape[1] != grad.shape[-1]:
...
model.mamba_unet.patch_embed.register_forward_hook(get_activation('patch_embed'))
model.mamba_unet.layers[0].register_forward_hook(get_activation('shallow'))
model.mamba_unet.layers[-1].register_forward_hook(get_activation('deep'))

# 运行一次 forward
features, x_downsample = model.mamba_unet.forward_features(input_img)
...
# 计算浅层、深层 Grad-CAM
# -----------------------
gradcam_shallow = GradCAM(model.mamba_unet, target_layer=model.mamba_unet.layers[0])
shallow_cam = gradcam_shallow(input_img, target_class=[1, 2, 3])  # (1,H,W)
shallow_cam = shallow_cam[0]  # -> (H,W)

gradcam_deep = GradCAM(model.mamba_unet, target_layer=model.mamba_unet.layers[-1])
...
# -- 关键改动：在 applyColorMap 前先反转灰度值
# 原本: shallow_cam_color = cv2.applyColorMap(shallow_cam_uint8, cv2.COLORMAP_JET)
# 改成:
shallow_cam_uint8_inv = 255 - shallow_cam_uint8
shallow_cam_color = cv2.applyColorMap(shallow_cam_uint8, cv2.COLORMAP_JET)
...
# 注册解码器特征的 hook
# 这里使用 layers_up 的第一个模块作为解码器浅层，最后一个模块作为解码器深层
model.mamba_unet.layers_up[1].register_forward_hook(get_activation('decoder_shallow'))
model.mamba_unet.layers_up[3].register_forward_hook(get_activation('decoder_deep'))

# 运行完整的前向传播，这样解码器部分也会执行，从而触发 hook
output = model(input_img)

# 从 activation 字典中获取解码器特征，假设它们已经转换为 (B, C, H, W)
...
gradcam_decoder_shallow = GradCAM(model.mamba_unet, target_layer=model.mamba_unet.layers_up[0])
...
# 解码器深层 CAM
gradcam_decoder_deep = GradCAM(model.mamba_unet, target_layer=model.mamba_unet.layers_up[-1])
...
if decoder_shallow_cam_color.shape[:2] != decoder_shallow_bg_color.shape[:2]:
    decoder_shallow_cam_color = cv2.resize(decoder_shallow_cam_color, (decoder_shallow_bg_color.shape[1], decoder_shallow_bg_color.shape[0]))
if decoder_deep_cam_color.shape[:2] != decoder_deep_bg_color.shape[:2]:
    decoder_deep_cam_color = cv2.resize(decoder_deep_cam_color, (decoder_deep_bg_color.shape[1], decoder_deep_bg_color.shape[0]))

alpha = 0.5
decoder_shallow_overlay = cv2.addWeighted(decoder_shallow_bg_color, alpha, decoder_shallow_cam_color, 1 - alpha, 0)
decoder_deep_overlay = cv2.addWeighted(decoder_deep_bg_color, alpha, decoder_deep_cam_color, 1 - alpha, 0)

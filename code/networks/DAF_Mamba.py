...
class pre_AFFM(nn.Module):
    ...
    def forward(self, local_features, global_features):
        ...
        # 如果 global_features 是全局平均池化后的结果，需要扩展到与 local_features 匹配的形状
        ...
        # 通过全连接层计算权重
        weights = self.softmax(self.fc(combined_features))
        print(f"weights shape: {weights.shape}")

        # 将权重拆分为局部和全局部分
        local_weight, global_weight = weights.chunk(2, dim=-1)
        print(f"local_weight shape: {local_weight.shape}")
        print(f"global_weight shape: {global_weight.shape}")

        # **关键修复**: 确保权重的形状与特征形状匹配
        fused_features = local_features * local_weight + global_features * global_weight
...
class pre_DMSM(nn.Module):
    def __init__(self, input_dim, scales):
        ...
        self.mamba_layers = nn.ModuleList([
            nn.Linear(input_dim, input_dim // (2 ** i)) for i in range(scales)
        ])
        # 确保 output_layer 的输入维度与 combined 的形状匹配
        self.output_layer = nn.Linear(sum(input_dim // (2 ** i) for i in range(scales)), input_dim)

    def forward(self, features):
        scale_outputs = [layer(features) for layer in self.mamba_layers]
        combined = torch.cat(scale_outputs, dim=-1)

        # 打印 combined 的形状以进行调试
...
class AFFM(pre_AFFM):
    def __init__(self, input_dim, adaptive_rate=0.3):
        ...
        self.adaptive_rate = adaptive_rate  # 自适应调节率
        self.fc = nn.Linear(input_dim * 2, input_dim * 2)  # 仍然使用全连接层，但可以动态调整输出

    def forward(self, local_features, global_features):
        B, H, W, C = local_features.shape

        # 自适应地调整全连接层的输出维度
        adaptive_dim = int(C * (1 + self.adaptive_rate))  # 动态调整维度
...
class DMSM(pre_DMSM):
    def __init__(self, input_dim, scales, dynamic_factor=1.0):
        ...
        self.dynamic_factor = dynamic_factor  # 动态因子，控制多尺度的变化
        self.mamba_layers = nn.ModuleList([
            nn.Linear(input_dim, input_dim // (2 ** i)) for i in range(scales)
        ])
        # 计算动态调整后的输出维度
        self.output_layer = nn.Linear(sum(input_dim // (2 ** i) for i in range(scales)),
                                      int(input_dim * self.dynamic_factor))

    def forward(self, features):
        # 根据输入特征的维度动态调整尺度
        dynamic_scales = int(features.size(-1) * self.dynamic_factor)
        scale_outputs = [layer(features) for layer in self.mamba_layers[:dynamic_scales]]  # 只使用前几个尺度
        combined = torch.cat(scale_outputs, dim=-1)
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2],
        ...
        super().__init__()
        # 保存输入参数，模型设计中需要的维度和超参数

        self.sf_mamba = AFFM(input_dim=dims[-1])
        self.mf_mamba = DMSM(input_dim=dims[0], scales=3)

        self.num_classes = num_classes
        self.num_layers = len(depths)  # 总共的网络层数
        if isinstance(dims, int):
            dims = [int(dims * 2 ** i_layer) for i_layer in range(self.num_layers)]
        self.embed_dim = dims[0]  # 初始特征嵌入维度
        self.num_features = dims[-1]  # 编码器最后一层的维度
        self.num_features_up = int(dims[0] * 2)  # 解码器最终的输出维度
        self.dims = dims
        self.final_upsample = final_upsample  # 最终上采样模式

        # 初始化 patch embedding 模块，用于将输入图像分块并转化为嵌入向量
    ...
    def forward_features(self, x):
        ...
        # Adding SF-Mamba after encoding
        local_features = x
        global_features = x.mean(dim=(1, 2), keepdim=True).expand_as(x)
        x = self.sf_mamba(local_features, global_features)

        x = self.norm(x)  # Normalize final output
    ...
    def forward_up_features(self, x, x_downsample):
        ...
        # Adding MF-Mamba for multi-scale feature integration
        x = self.mf_mamba(x)

        x = self.norm_up(x)  # Normalize final output
        return x

    # 最终上采样模块，将特征映射为最终的输出分辨率
    def up_x4(self, x):
        if self.final_upsample == "expand_first":
            B, H, W, C = x.shape
            x = self.up(x)  # x4 上采样
            x = x.view(B, 4 * H, 4 * W, -1)
            x = x.permute(0, 3, 1, 2)  # 调整为 B, C, H, W 格式
            x = self.output(x)  # 最后卷积输出

        return x

    # 整体前向传播：编码、解码、上采样
    def forward(self, x):
        x, x_downsample = self.forward_features(x)  # 编码阶段
        x = self.forward_up_features(x, x_downsample)  # 解码阶段
        x = self.up_x4(x)  # 上采样阶段
        return x

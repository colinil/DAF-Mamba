...
class BaseDataSets(Dataset):
    def __init__(
        ...
        # Reading training and validation data from 2D slices
        if self.split == "train":
            with open(self._base_dir + "/train.list", "r") as f1:  # Assuming 2D slices list for training
        ...
        # Restrict the number of samples if needed
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            h5f = h5py.File(self._base_dir + "/data/train/{}.h5".format(case), "r")  # Handle 2D slices
        else:
            h5f = h5py.File(self._base_dir + "/data/{}.h5".format(case), "r")  # Handle 2D slices

        image = h5f["image"][:]  # This will load a 2D image
        label = h5f["label"][:]

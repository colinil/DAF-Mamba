...
def Inference(FLAGS):
    ...
    test_save_path = "../model/{}_{}_labeled/{}_predictions-999/".format(FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
    ...
    # 用于保存每个病例的 hd95 值，列表中每个元素为 [case, hd95_class1, hd95_class2, hd95_class3]
    hd95_results = []

    for case in tqdm(image_list):
        first_metric, second_metric, third_metric = test_single_volume(case, net, test_save_path, FLAGS)
        # 保存每个病例的 hd95 值（各类别指标中的第三项）
        avg_hd95 = (first_metric[2] + second_metric[2] + third_metric[2]) / 3.0
        #hd95_results.append([case, first_metric[2], second_metric[2], third_metric[2], avg_hd95])
        hd95_results.append([case,avg_hd95])
    ...
    # 将 hd95 结果保存为 csv 文件
    csv_path = os.path.join(test_save_path, "hd95_results.csv")
    with open(csv_path, mode='w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(["Case", "HD95_Average"])
        csv_writer.writerows(hd95_results)
    print("HD95 results have been saved to:", csv_path)

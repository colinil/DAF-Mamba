...
def Inference(FLAGS):
    ...
    test_save_path = "../model/{}_{}_labeled/{}_predictions222no/".format(FLAGS.exp, FLAGS.labeled_num, FLAGS.model)
    ...
    # 用于存储每个测试文件的case和hd95平均值
    hd95_list = []
    for case in tqdm(image_list):
        metrics = test_single_image(case, net, test_save_path, FLAGS)
        hd95_values = []
        for i in range(FLAGS.num_classes - 1):
            hd_value = metrics[i][2]
            hd95_values.append(hd_value)
        # 计算三个类别的平均hd95
        hd95_avg = np.mean(hd95_values)
        hd95_list.append({"case": case, "hd95_avg": hd95_avg})

        for i in range(FLAGS.num_classes - 1):
            total_metrics[i] += np.asarray(metrics[i])

    # 保存hd95平均结果到CSV文件，只包含case和hd95_avg两栏
    csv_file = os.path.join(test_save_path, "hd95_results.csv")
    with open(csv_file, mode='w', newline='') as csvfile:
        fieldnames = ["case", "hd95_avg"]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        for entry in hd95_list:
            writer.writerow(entry)
    print(f"hd95平均结果已保存到: {csv_file}")

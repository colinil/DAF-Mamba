...
def calculate_metric_percase(pred, gt):
    ...
    spacing_2d = (0.308, 0.308)
    dice = metric.binary.dc(pred, gt)
    asd = metric.binary.asd(pred, gt, voxelspacing=spacing_2d)
    hd95 = metric.binary.hd95(pred, gt, voxelspacing=spacing_2d)
...
def test_single_image(case, net, test_save_path, FLAGS):
    ...
    #slice = np.clip(slice_resized, 0, 1)  # 处理图像范围
...
def Inference(FLAGS):
    ...
    save_mode_path = os.path.join(snapshot_path, '{}_best_model2.pth'.format(FLAGS.model))
